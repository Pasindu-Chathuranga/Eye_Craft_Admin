...
const ItemPanel = () => {
    ...
    return (
        <Box sx={{ bgcolor: 'rgba(255,255,255,0.8)', borderRadius: '10px ', width: '100%', position: 'relative' }}>
            <Grid container justifyContent="start" sx={{ zIndex:'999', position: 'fixed', bgcolor: 'background.paper', borderRadius: '10px 10px 0px 0px', width: '98%', boxShadow: ' 2px 2px 10px 0 rgba(0, 0, 0, 0.5)' }}>
                {/* Header and Add Button on the same row */}
                <Grid item xs={12} p={3} sx={{ borderBottom: '1px #1c1c1c solid', paddingBottom: '10px' }}>
                    <Grid container alignItems="center" justifyContent="space-between">
                        <Typography variant="h3" className="item-panel-header-title" sx={{ fontSize: '1.5rem' }}> {visibility ? 'Manage item' : `Items | showing ${filteredItems.length} / ${items.length}`}</Typography>
